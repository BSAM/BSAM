function[] = surfplot3d(s0,nn,var,varc,thres,toggle1,toggle2)
%
...
% varc is the variable number of the coloring
% thres is the isosurface of var to print
...
[A]=fscanf(f,'%f', [nrvars,(n(1)+2*mbc)*(n(2)+2*mbc)*(n(3)+2*mbc)]); % ghost layer included.
    ...
    if level > (maxlevel-3)
  drawsurf(A,var,varc,n,xl,xu,dx,xlg,xug,mbc,thres);
    end;
  else
    theend = 1;
  end
...
%text(-1.1,-1.1,1.1+2.2/24.0,['time = ' num2str(time)], 'FontSize', 14);
...
elseif toggle1 == 2
  print('-depsc','-r400',OUT)
end;

ipatch
fclose(f);

function drawsurf(A,var,varc,n,xl,xu,dx,xlg,xug,mbc,thres)

%c = zeros(n(1)+2,n(2)+2,n(3)+2,'double');
c = zeros(n(2)+2*mbc,n(1)+2*mbc,n(3)+2*mbc,'double');
cr = zeros(n(2)+2*mbc,n(1)+2*mbc,n(3)+2*mbc,'double');

for k=1:n(3)+2*mbc
  for j=1:n(2)+2*mbc
    for i=1:n(1)+2*mbc

%      c(i,j,k) = A(1,((k-1)*(n(2)+2*mbc)+j-1)*(n(1)+2*mbc)+i);
       c(j,i,k) = A(var,((k-1)*(n(2)+2*mbc)+j-1)*(n(1)+2*mbc)+i);
       cr(j,i,k) = A(varc,((k-1)*(n(2)+2*mbc)+j-1)*(n(1)+2*mbc)+i);
    end;
  end;
end;

[xx,yy,zz]=meshgrid(linspace(xl(1)-dx(1)/2,xu(1)+dx(1)/2,n(1)+2*mbc), ...
                    linspace(xl(2)-dx(2)/2,xu(2)+dx(2)/2,n(2)+2*mbc), ...
                    linspace(xl(3)-dx(3)/2,xu(3)+dx(3)/2,n(3)+2*mbc));

p = patch(isosurface(xx, yy, zz, c, thres,cr));
