function[] = surfplot3d(s0,nn,var,varc,toggle1,toggle2)
%
...
[A]=fscanf(f,'%f', [nrvars,(n(1)+2*mbc)*(n(2)+2*mbc)*(n(3)+2*mbc)]); % ghost layer included.
    ...
    if level == maxlevel
  drawsurf(A,var,varc,n,xl,xu,dx,xlg,xug,mbc);
    end;
  else
    theend = 1;
  end
...
function drawsurf(A,var,varc,n,xl,xu,dx,xlg,xug,mbc)
...
p = patch(isosurface(xx, yy, zz, c, 0.5,cr));
